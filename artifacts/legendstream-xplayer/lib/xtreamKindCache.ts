...
import {
  beginXtreamCardinalityAttempt,
  finishXtreamCardinalityPublish,
  noteXtreamCardinalityPublishCalled,
  noteXtreamCardinalityStagedCount,
} from "./xtreamCardinalityDiagnostics";
...
export function stagingCatalogProviderId(
  ...
  const stagingId = `${STAGING_PROVIDER_PREFIX}${providerId}:xtream:${generation}:${kind}`;
  beginXtreamCardinalityAttempt(stagingId, kind);
  return stagingId;
}
...
export async function publishStagedCatalogKind(
  ...
  noteXtreamCardinalityPublishCalled(options.stagingId, options.kind);
  return enqueueCatalogDbWrite(async () => {
    if (options.canPublish && !options.canPublish()) {
      finishXtreamCardinalityPublish(options.stagingId, options.kind, false, null);
    ...
    try {
      await db.withExclusiveTransactionAsync(async (txn) => {
        ...
        noteXtreamCardinalityStagedCount(options.stagingId, options.kind, stagedCount);
    ...
    finishXtreamCardinalityPublish(
      ...
      options.kind,
      result.published,
      result.published ? result.activeCount : null,
    );
    return result;
  });
}
