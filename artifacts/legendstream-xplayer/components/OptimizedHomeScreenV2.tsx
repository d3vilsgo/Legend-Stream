...
export default function OptimizedHomeScreenV2() {
  ...
  const goBackFromPlayer = () => setView(playable?.kind === "movie" ? "movies" : playable?.kind === "episode" ? "series" : "live");

  if (view === "player") {
    return <View style={s.fullPlayer}>{playable ? <NativeVideoPlayer source={playable.url} title={playable.title} autoFullscreen onFullscreenExit={goBackFromPlayer} /> : null}</View>;
  }
  ...
  return <View style={[s.screen, { backgroundColor: colors.background, paddingTop: top, paddingBottom: Math.max(insets.bottom, 10) }]}>
    ...
    <ScrollView style={{ flex: 1 }} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      {view === "library" ? <HistoryView channels={providerChannels} favorites={favorites} history={history} onOpen={openLive} /> : null}
      {view === "settings" ? <Settings provider={provider} providers={providers} onEdit={() => setEditing(true)} onSwitch={(id) => void setActiveProvider(id)} onLogout={() => void removeProvider(provider.id)} /> : null}
    </ScrollView>
  </View>;
}
...
function ProviderSetup({ existing, busy, error, onCancel, onSubmit }: {
  ...
  return <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
    <ScrollView contentContainerStyle={[s.setup, { paddingTop: insets.top + 24, paddingBottom: insets.bottom + 140 }]} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
      ...
      <Input label="Source name" value={name} onChangeText={setName} /><Input label="Server / playlist URL" value={url} onChangeText={setUrl} autoCapitalize="none" />
...
function Live({ channels, favorites, loading, onRefresh, onOpen, onFavorite }: { channels: Channel[]; favorites: string[]; loading: boolean; onRefresh: () => void; onOpen: (c: Channel) => void; onFavorite: (id: string) => void }) {
  ...
  // Preserve the exact category order returned by the provider; do not alphabetize it.
...
function HistoryView({ channels, favorites, history, onOpen }: { channels: Channel[]; favorites: string[]; history: string[]; onOpen: (c: Channel) => void }) {
  const colors = useColors();
  const recent = history.map((id) => channels.find((c) => c.id === id)).filter((x): x is Channel => Boolean(x));
  const favs = favorites.map((id) => channels.find((c) => c.id === id)).filter((x): x is Channel => Boolean(x));
  const section = (title: string, rows: Channel[]) => <View style={{ marginBottom: 28 }}><Text style={[s.section, { color: colors.foreground, marginBottom: 10 }]}>{title}</Text>{rows.length ? rows.map((c) => <Pressable key={`${title}-${c.id}`} onPress={() => onOpen(c)} style={[s.episode, { borderColor: colors.border, backgroundColor: colors.card }]}><View style={{ flex: 1 }}><Text style={{ color: colors.foreground, fontWeight: "700" }}>{c.name}</Text><Text style={{ color: colors.mutedForeground }}>{c.category}</Text></View><Feather name="play" size={20} color={colors.primary} /></Pressable>) : <Text style={{ color: colors.mutedForeground }}>Nothing here yet.</Text>}</View>;
