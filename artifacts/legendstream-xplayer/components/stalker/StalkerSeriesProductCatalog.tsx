...
export function StalkerSeriesProductCatalog({
  screen, categories, items, detail, selectedCategoryId, selectedCategoryTitle, selectedSeasonId,
  currentPage, totalItems, maxPageItems, hasNextPage, loading, loadingMore, error, pagingError,
  playbackLoading, playbackError, searchQuery, searchLoading, searchError, onSearchQueryChange,
  onBack, onRetry, onRetryNextPage, onSelectCategory, onSelectSeries, onSelectSeason, onSelectEpisode, onLoadMore,
}: {
  ...
  if (screen === "detail") {
    return <ScrollView style={styles.screen} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {!loading && !error && detail ? <View style={styles.detailSection}>
        ...
        {detail.seasons.length ? <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.seasonTabs}>
          {detail.seasons.map((season) => {
            const selected = season.id === selectedSeasonId;
            return <Pressable key={season.id} accessibilityRole="button" accessibilityLabel={season.label} accessibilityState={{ selected }} onPress={() => onSelectSeason(season.id)} style={[styles.seasonTab, { borderColor: selected ? colors.primary : colors.border, backgroundColor: selected ? colors.secondary : colors.card }]}>
        ...
        {selectedSeason ? <View style={styles.episodes}>
          ...
          {selectedSeason.episodes.length ? selectedSeason.episodes.map((episode) => <Pressable key={episode.key} accessibilityRole="button" accessibilityLabel={episode.label} onPress={() => onSelectEpisode(selectedSeason.id, episode.id)} style={[styles.episode, { borderColor: colors.border, backgroundColor: colors.card }]}>
    ...
    <View style={styles.titleRow}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={{ color: colors.mutedForeground }}>
          {screen === "categories" ? "Kategori seçin" : screen === "search" ? "Tüm dizi kataloğunda arama" : `${selectedCategoryTitle || "Diziler"}${totalItems != null ? ` · ${totalItems} dizi` : ""}${maxPageItems != null ? ` · sayfa başına ${maxPageItems}` : ""}`}
        </Text>
      </View>
    </View>
  ...
  if (screen === "categories") {
    return <FlatList
      ...
      renderItem={({ item }) => {
        ...
        return <Pressable accessibilityRole="button" accessibilityLabel={item.title} accessibilityState={{ selected }} onPress={() => onSelectCategory(item.id)} style={[styles.card, { borderColor: selected ? colors.primary : colors.border, backgroundColor: colors.card }]}>
  ...
  return <FlatList
    key={`stalker-series-grid-${screen}`}
    style={styles.screen}
    contentContainerStyle={styles.content}
    data={items}
    keyExtractor={(item) => item.id}
    ...
    ListHeaderComponent={header}
    ListEmptyComponent={!loading && !searchLoading && !error && !searchError ? <Empty text={screen === "search" ? "Aramanızla eşleşen dizi bulunamadı." : "Bu kategoride dizi bulunamadı."} /> : null}
    renderItem={({ item }) => <Pressable accessibilityRole="button" accessibilityLabel={item.title} onPress={() => onSelectSeries(item.id)} style={[styles.seriesCard, { borderColor: colors.border, backgroundColor: colors.card }]}>
      <Poster item={item} />
      <Text numberOfLines={2} style={[styles.seriesTitle, { color: colors.foreground }]}>{item.title}</Text>
      <Text numberOfLines={1} style={{ color: colors.mutedForeground, fontSize: 12 }}>{[item.year, item.rating].filter(Boolean).join(" · ")}</Text>
    </Pressable>}
    ...
    ListFooterComponent={screen === "list" ? <View style={styles.footer}>
      ...
      {pagingError ? <View style={styles.footer}><ErrorCard detail={pagingError} onRetry={onRetryNextPage} /></View> : null}
...
function SearchBox({ value, placeholder, onChange }: { value: string; placeholder: string; onChange: (value: string) => void }) {
  ...
  return <View style={[styles.searchBox, { borderColor: colors.border, backgroundColor: colors.card }]}>
    ...
    <TextInput value={value} onChangeText={onChange} placeholder={placeholder} placeholderTextColor={colors.mutedForeground} autoCorrect={false} style={[styles.searchInput, { color: colors.foreground }]} />
    {value ? <Pressable accessibilityRole="button" accessibilityLabel="Aramayı temizle" onPress={() => onChange("")}><Feather name="x-circle" size={20} color={colors.mutedForeground} /></Pressable> : null}
  </View>;
}
...
function Poster({ item, large = false }: { item: Pick<StalkerSeriesProductItem, "posterUrl" | "title">; large?: boolean }) {
  ...
  return <View style={[style, { backgroundColor: colors.secondary }]}>
    {item.posterUrl ? <Image source={{ uri: item.posterUrl }} style={styles.posterImage} resizeMode="cover" /> : <View style={styles.posterFallback}><Feather name="tv" size={large ? 34 : 26} color={colors.mutedForeground} /><Text style={{ color: colors.mutedForeground }}>Dizi</Text></View>}
  </View>;
}

function StateCard({ loading, text }: { loading?: boolean; text: string }) {
  const colors = useColors();
  return <View style={[styles.stateCard, { borderColor: colors.border, backgroundColor: colors.card }]}>{loading ? <ActivityIndicator size="small" color={colors.primary} /> : null}<Text style={{ color: colors.mutedForeground }}>{text}</Text></View>;
}
...
function ErrorCard({ detail, onRetry }: { detail: string; onRetry: () => void }) {
  ...
  return <View style={[styles.stateCard, { borderColor: colors.destructive, backgroundColor: colors.card }]}>
    <View style={{ flex: 1, gap: 3 }}><Text style={{ color: colors.destructive, fontWeight: "800" }}>Diziler yüklenemedi</Text><Text style={{ color: colors.mutedForeground }}>{detail}</Text></View>
    <FocusButton label="Tekrar dene" icon="refresh-cw" variant="secondary" onPress={onRetry} />
  </View>;
}

function Empty({ text }: { text: string }) { return <StateCard text={text} />; }
