...
import { beginStalkerPlaybackTrace, clearStalkerTraceEntries, describePlaybackSourceSafely, formatStalkerTrace, getStalkerTraceEntries, traceStalker } from "../lib/stalkerPlaybackTrace";
...
const secretUrl="http://user:pass@example.invalid/live/token/196699.ts?token=super-secret&x=1"; const metadata=describePlaybackSourceSafely(secretUrl); assert.equal(metadata.scheme,"http"); assert.equal(metadata.looksLikeTransportStream,true); assert.equal(metadata.hasQuery,true); assert.equal(metadata.queryKeyCount,2); assert.equal(JSON.stringify(metadata).includes(secretUrl),false); assert.equal(JSON.stringify(metadata).includes("example.invalid"),false); assert.equal(JSON.stringify(metadata).includes("super-secret"),false);
